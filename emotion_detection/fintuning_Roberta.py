...
if __name__ == '__main__':
    ...
    #  unique labels
    UNIQUE_LABELS_TRAIN = list(sorted(DF_TRAIN['label'].unique()))
    
    TRAIN_DATASET, VAL_DATASET, TEST_DATASET = huggingface_label2int(TRAIN_DATASET, VAL_DATASET, TEST_DATASET, UNIQUE_LABELS_TRAIN)

    LABEL_LIST = list(sorted(DF_TRAIN['label'].unique()))

    LABEL2ID = {label: i for i, label in enumerate(LABEL_LIST)}
    ID2LABEL = {v: k for k, v in LABEL2ID.items()}

    # Update the model's configuration with the id2label mapping
    CONFIG = AutoConfig.from_pretrained(config.xlm_roberta_model)
    CONFIG.update({"id2label": ID2LABEL})

    # Preprocessing
    TOKENIZER = RobertaTokenizerFast.from_pretrained(config.xlm_roberta_model)
    TRAIN_DATASET, VAL_DATASET, TEST_DATASET = huggingface_proceed(TRAIN_DATASET, VAL_DATASET, TEST_DATASET, TOKENIZER)

    # Model
    MODEL = RobertaForSequenceClassification.from_pretrained(config.xlm_roberta_model, num_labels=5)
    
    print("Model name: ", config.xlm_roberta_model)

    # TrainingArguments
    TRAINING_ARGS = TrainingArguments(
        output_dir= config.xlm_roberta_model,
        num_train_epochs=3,
    ...
    MODEL.save_pretrained(config.finetuned_xlm_roberta_dir)